  ...
  useEffect(() => {
    apiClient.getReceivingPoints().then((res) => {
      if (res.success && res.data) {
        ...
        const myBranch = res.data.find((b) => b.id === user?.receivingPoint?.id);
        if (myBranch) setBranchId(myBranch.id);
        else if (res.data.length > 0) setBranchId(res.data[0].id);
      }
    });
  }, [user]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Vault, tellers, disbursements, reconciliation and expenses for the period</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
        <div className="flex gap-3 flex-wrap items-center">
          <select value={branchId} onChange={(e) => setBranchId(e.target.value)} className={`${inputCls} w-52`}>
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Variance (Net)',   value: fmtGHS(data.reconciliation.totalVariance), sub: `${data.reconciliation.completed + data.reconciliation.approved} resolved recons`, color: Math.abs(data.reconciliation.totalVariance) < 0.01 ? 'text-green-700' : 'text-amber-700', bg: Math.abs(data.reconciliation.totalVariance) < 0.01 ? 'bg-green-50' : 'bg-amber-50', border: Math.abs(data.reconciliation.totalVariance) < 0.01 ? 'border-green-100' : 'border-amber-100' },
            ].map((kpi) => (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-50">
                    ...
                    <th className="text-right py-2 px-4 text-[11px] font-semibold uppercase tracking-wider text-gray-400">Till Balance</th>
                  </tr>
                </thead>
                ...
                <tbody className="divide-y divide-gray-50">
                  {data.expenses.map((exp) => (
                    <tr key={exp.accountCode} className="hover:bg-gray-50/60 transition-colors">
                      <td className="py-3 px-4 font-mono text-xs text-blue-600 font-bold w-32">{exp.accountCode}</td>
