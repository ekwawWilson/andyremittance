...
import { fmtNum, fmtCADSigned } from '@/lib/utils/format';
...
export default function GeneralLedgerPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">General Ledger</h1>
        <p className="text-sm text-gray-400 mt-0.5">Detailed transaction history for any account with running balance</p>
      </div>
        ...
        <div className="flex gap-3 flex-wrap items-end">
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              placeholder="e.g. CASH-CAD, DISBURSE-EXPENSE…"
        ...
        <>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs text-gray-400 font-medium uppercase tracking-widest mb-1">Account</p>
                <h2 className="text-lg font-bold text-gray-900">
                  <span className="font-mono text-blue-600 mr-2">{result.account.accountCode}</span>
                  {result.account.accountName}
                </h2>
                <p className="text-sm text-gray-400 mt-0.5">
                  {result.account.accountType} · {result.account.currency}
                  {result.account.receivingPoint && ` · ${result.account.receivingPoint.name}`}
                </p>
              </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-4">
              {[
                { label: 'Total Debits',  value: result.totalDebits,  color: 'text-blue-600' },
                { label: 'Total Credits', value: result.totalCredits, color: 'text-green-600' },
                { label: 'Net Movement',  value: result.netMovement,  color: result.netMovement < 0 ? 'text-red-600' : 'text-gray-800' },
              ].map((kpi) => (
                <div key={kpi.label} className="bg-gray-50 rounded-xl p-3 text-center">
                  ...
                  <p className={`text-base font-bold mt-0.5 ${kpi.color}`}>
                    {result.account.currency} {Number(kpi.value).toLocaleString('en-GH', { minimumFractionDigits: 2 })}
                  </p>
                </div>
              ))}
            </div>
              ...
              {result.ledger.length > 0 && (
                <tfoot>
                  <tr className="bg-gray-50/60 border-t border-gray-100">
                    ...
                    <td className="py-2 px-4 text-right font-bold text-sm text-blue-700">{fmtNum(result.totalDebits)}</td>
                    <td className="py-2 px-4 text-right font-bold text-sm text-green-700">{fmtNum(result.totalCredits)}</td>
